...
CPredictionSystem::CPredictionSystem() {
	m_nPredictionRandomSeed = *reinterpret_cast<int**>(SSDK::O::FindSignature(XorStr("client.dll"), XorStr("8B 0D ? ? ? ? BA ? ? ? ? E8 ? ? ? ? 83 C4 04")) + 0x2);
	m_pPredictionPlayer = *reinterpret_cast<CBaseEntity**>(SSDK::O::FindSignature(XorStr("client.dll"), XorStr("89 3D ? ? ? ? F3 0F 10 48 ? ")) + 0x2);
}
